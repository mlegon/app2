...
(función (global, fábrica) {
	...
	if ( tipo de módulo === "objeto" && tipo de módulo.exportaciones === "objeto" ) {
		// Para entornos similares a CommonJS y CommonJS donde hay una ventana adecuada,
		// ejecuta la fábrica y obtiene jQuery
		// Para entornos que no poseen inherentemente una ventana con un documento
		// (como Node.js), exponer una fábrica de creación de jQuery como module.exports
...
// No puedo hacer esto porque hay varias aplicaciones, incluido el seguimiento de ASP.NET
// la pila a través de arguments.caller.callee y Firefox muere si
// intenta rastrear a través de cadenas de llamadas de "uso estricto". (#13335)
// Soporte: Firefox 18+
//

var identificadores eliminados = [];

var segmento = IDs eliminados.segmento;

var concat = identificadores eliminados.concat;

var push = ID eliminados.push;
...
variable
	...
	// Usado por jQuery.camelCase como devolución de llamada para replace()
...
jQuery.fn = jQuery.prototipo = {
	...
	cada: función (devolución de llamada, argumentos) {
		devuelve jQuery.each (esto, devolución de llamada, argumentos);
	},
	...
	final: función () {
		devolver este.prevObject || este.constructor(nulo);
	},

	// Sólo para uso interno.
	// Se comporta como un método de Array, no como un método de jQuery.
	empuja empuja,
	ordenar: deleteIds.sort,
...
jQuery.extend = jQuery.fn.extend = función() {
	...
	if ( tipo de destino !== "objeto" && !jQuery.isFunction(objetivo) ) {
		objetivo = {};
	}

	// extender jQuery si solo se pasa un argumento
	si ( i === longitud ) {
		objetivo = esto;
		I--;
	}
		...
		if ((opciones = argumentos[ i ]) != null ) {
			...
			para (nombre en opciones) {
				...
				if ( deep && copy && ( jQuery.isPlainObject(copy) || (copyIsArray = jQuery.isArray(copy)) ) ) {
					if (copiarArray) {
						copyIsArray = falso;
						clonar = src && jQuery.isArray(src) ? origen: [];

					} demás {
						clonar = src && jQuery.isPlainObject(src) ? origen: {};
					}
...
jQuery.extender({
	...
	esFuncion: funcion( obj ) {
		return jQuery.type(obj) === "función";
	},

	esArray: Array.isArray || función( objeto ) {
		devuelve jQuery.type(obj) === "matriz";
	},

	esVentana: función( obj ) {
		/* jshint eqeqeq: falso */
		return obj != nulo && obj == obj.ventana;
	},

	esnumérico: función( obj ) {
		// parseFloat NaNs conversión numérica de falsos positivos (nulo|verdadero|falso|"")
	...
	esPlainObject: function( obj ) {
		...
		tratar {
			// La propiedad del constructor no propio debe ser Object
			if (obj.constructor &&
		...
		// Soporte: IE<9
		// Manejar la iteración sobre las propiedades heredadas antes que las propiedades propias.
		if (apoyo.propioÚltimo) {
			para (ingrese obj) {
				return hasOwn.call( obj, clave );
			}
		}

		// Las propiedades propias se enumeran en primer lugar, por lo que para acelerar,
		// si el último es propio, entonces todas las propiedades son propias.
		para (ingresar obj) {}

		tecla de retorno === indefinido || tienePropio.llamada( obj, clave );
	},
	...
	globalEval: función (datos) {
		if (datos && jQuery.trim(datos)) {
			...
			(ventana.execScript || función (datos) {
	...
	hacerArray: function(arr, resultados) {
		...
		if ( arr != null ) {
			if ( es como una matriz (objeto (arr))) {
				jQuery.merge(ret,
					typeof arr === "cadena" ?
					[arr]: arr
				);
	...
	enArray: function( elem, arr, i ) {
		...
		si ( arr ) {
			...
			para ( ; i < len; i++ ) {
				// Omitir el acceso en arreglos dispersos
	...
	combinar: función (primero, segundo) {
		...
		// Soporte: IE<9
		// Solución de conversión de .length a NaN en objetos similares a matrices (por ejemplo, NodeLists)
		if ( largo !== largo ) {
...
función es como una matriz ( obj ) {
...
var Chisporrotear =
/*!
 * Chisporrotea el motor selector CSS v1.10.19
 * http://sizzlejs.com/
 *
...
var yo,
	...
	expando = "chisporrotear" + -(nueva fecha()),
	...
	strundefined = tipo de indefinido,
	MAX_NEGATIVO = 1 << 31,
	...
	// Usa un indexOf simplificado si no podemos usar uno nativo
	indexOf = arr.indexOf || función( elemento ) {
		var i = 0,
			len = este.longitud;
		para ( ; i < len; i++ ) {
			si (este[i] === elemento) {
	...
	booleanos = "marcado|seleccionado|asincrónico|enfoque automático|reproducción automática|controles|aplazado|deshabilitado|oculto|ismap|bucle|múltiple|abierto|solo lectura|requerido|ámbito",

	// Expresiones regulares

	// Caracteres de espacio en blanco http://www.w3.org/TR/css3-selectors/#whitespace
	espacio en blanco = "[\\x20\\t\\r\\n\\f]",
	// http://www.w3.org/TR/css3-syntax/#caracteres
	codificación de caracteres = "(?:\\\\.|[\\w-]|[^\\x00-\\xa0])+",
	...
	identificador = codificación de caracteres.reemplazar ("w", "w #"),

	// Selectores de atributos: http://www.w3.org/TR/selectors/#attribute-selectors
	atributos = "\\[" + espacios en blanco + "*(" + codificación de caracteres + ")(?:" + espacios en blanco +
		// Operador (captura 2)
		"*([*^$|!~]?=)" + espacio en blanco +
	...
	coincidenciaExpr = {
		"ID": nueva RegExp( "^#(" + codificación de caracteres + ")" ),
		"CLASE": nueva RegExp( "^\\.(" + codificación de caracteres + ")" ),
		"ETIQUETA": nueva RegExp( "^(" + codificación de caracteres. reemplazar ("w", "w*" ) + ")" ),
		"ATTR": nueva expresión regular ("^" + atributos),
		"PSEUDO": nueva expresión regular ("^" + pseudos),
		"CHILD": new RegExp( "^:(solo|primero|último|nésimo|enésimo-último)-(hijo|de-tipo)(?:\\(" + espacios en blanco +
		...
		"needsContext": new RegExp( "^" + espacio en blanco + "*[>+~]|:(par|impar|eq|gt|lt|nth|primero|último)(?:\\(" +
	...
	funescape = function( _, escapado, espacio en blanco escapado ) {
		var alto = "0x" + escapado - 0x10000;
		// NaN significa sin punto de código
		// Soporte: Firefox<24
		// Solución de interpretación numérica errónea de +"0x"
		volver alto !== alto || espacio en blanco escapado?
			escapado :
			alto < 0 ?
				// punto de código BMP
...
tratar {
	empujar.aplicar(
		(arr = segmento.llamada(preferidoDoc.childNodes)),
...
función Sizzle (selector, contexto, resultados, semilla) {
	coincidencia de var, elemento, m, tipo de nodo,
		// variables QSA
		i, grupos, viejo, nid, newContext, newSelector;

	if (( contexto ? contexto.documentopropietario || contexto : documentopreferido ) !== documento ) {
	...
	if ( !selector || tipode selector !== "cadena" ) {
		devolver resultados;
	}

	if ( (tipo de nodo = contexto.tipo de nodo) !== 1 && tipo de nodo !== 9 ) {
		regreso [];
	}
	...
	if (documentIsHTML && !seed) {
		...
		if ((coincidencia = rquickExpr.exec(selector))) {
			...
			si ( (m = partido[1]) ) {
				if ( tipo de nodo === 9 ) {
					...
					if ( contexto.Documentopropietario && (elemento = contexto.Documentopropietario.getElementById( m )) &&
		...
		// ruta QSA
		if ( soporte.qsa && (!rbuggyQSA || !rbuggyQSA.prueba( selector )) ) {
			nid = viejo = expando;
			nuevoContexto = contexto;
			nuevoSelector = tipoNodo === 9 && selector;

			// qSA funciona de manera extraña en consultas basadas en elementos
			// Podemos solucionar esto especificando una ID adicional en la raíz
			// y trabajando a partir de ahí (Gracias a Andrew Dupont por la técnica)
			// IE 8 no funciona en elementos de objetos
...
función siblingCheck( a, b ) {
	var cur = b && a,
		diff = cur && a.nodeType === 1 && b.nodeType === 1 &&
			( ~b.índice de origen || MAX_NEGATIVE ) -
...
isXML = Chisporroteo.isXML = función (elemento) {
	...
	devolver documentoElemento? documentElement.nodeName !== "HTML" : false;
};
...
setDocument = Sizzle.setDocument = función (nodo) {
	var hasCompare,
		...
		padre = doc. vista por defecto;

	// Si no hay documento ni documentElement disponible, devuelve
	...
	// Soporte: IE>8
	// Si el documento iframe está asignado a la variable "documento" y si iframe ha sido recargado,
	...
	if ( padre && padre !== padre.superior ) {
		// IE11 no tiene un evento adjunto, por lo que todos deben sufrir
		if (padre.addEventListener) {
			parent.addEventListener( "descargar", function() {
				establecerDocumento();
			}, falso );
		} más si (padre.adjuntarEvento) {
	...
	// Soporte: IE<8
	// Verifica que getAttribute realmente devuelva atributos y no propiedades (excepto los booleanos de IE8)
	soporte.atributos = afirmar (función (div) {
		div.className = "i";
		volver !div.getAttribute("nombreClase");
	});
	...
	soporte.getElementsByTagName = afirmar (función (div) {
		div.appendChild( doc.createComment("") );
		volver !div.getElementsByTagName("*").longitud;
	});
	...
	support.getElementsByClassName = rnative.test( doc.getElementsByClassName ) && assert(function( div ) {
		...
		// Soporte: Safari<4
		// Captura el exceso de almacenamiento en caché de la clase
		div.firstChild.className = "i";
		// Soporte: Ópera<10
		// Captura la falla de gEBCN para encontrar clases no líderes
		devuelve div.getElementsByClassName("i").length === 2;
	});
	...
	soporte.getById = afirmar (función (div) {
		docElem.appendChild(div).id = expando;
		volver !doc.getElementsByName || !doc.getElementsByName( expando ).longitud;
	});
	...
	if ( (soporte.qsa = rnative.test( doc.querySelectorAll )) ) {
		...
		afirmar (función (div) {
			...
			div.innerHTML = "<select msallowclip=''><opción seleccionada=''></opción></select>";
			...
			if ( !div.querySelectorAll(":marcado").longitud ) {
		...
		afirmar (función (div) {
			...
			var entrada = doc.createElement("entrada");
			...
			// IE8 arroja un error aquí y no verá pruebas posteriores
			if ( !div.querySelectorAll(":habilitado").longitud ) {
	...
	// A propósito, no implementa el descendiente inclusivo
	...
	función( a, b ) {
		...
		si ( comparar & 1 ||
			...
			if ( b === doc || b.documentopropietario === documentopreferido && contiene(docpreferido, b) ) {
				devolver 1;
			}
...
Sizzle.matchesSelector = función (elemento, expr) {
	...
	// Asegúrese de que los selectores de atributos estén entre comillas
...
Expr = Sizzle.selectors = {
	...
	prefiltro: {
		"ATTR": función (coincidencia) {
			...
			partido[3] = ( partido[3] || partido[4] || partido[5] || "" ).replace( runescape, funescape );
		...
		"NIÑO": función (coincidencia) {
			...
			*/
			...
			if (coincidencia[1].segmento( 0, 3 ) === "nth" ) {
				...
				coincidencia[4] = +( coincidencia[4] ? coincidencia[5] + (coincidencia[6] || 1) : 2 * (coincidencia[3] === "par" || coincidencia[3] === " impar" ) );
		...
		"PSEUDO": función (coincidencia) {
			...
			si (coincidencia[3]) {
				coincidencia[2] = coincidencia[4] || partido[5] || "";
		...
		"NIÑO": función (tipo, qué, argumento, primero, último) {
			...
			volver primero === 1 && último === 0 ?
				...
				función (elemento, contexto, xml) {
					var caché, caché externo, nodo, diferencia, índice de nodo, inicio,
					...
					si (padre) {
						...
						if (adelante && useCache) {
							...
							cachéexterno = padre[ expando ] || (padre[ expando ] = {});
							caché = caché externo[ tipo ] || [];
			...
			if ( fn.longitud > 1 ) {
				...
				devolver Expr.setFilters.hasOwnProperty( pseudo.toLowerCase() ) ?
					markFunction(función( semilla, coincidencias ) {
						...
						mientras yo-- ) {
							idx = indexOf.call (seed, emparejado [i]);
							semilla[ idx ] = !(coincidencias[ idx ] = emparejado[i] );
						}
					}):
					función( elemento ) {
	...
	pseudos: {
		...
		"no": marcarFunción(función(selector) {
			...
			devolver emparejador[ expando ] ?
				markFunction(función( semilla, coincidencias, contexto, xml ) {
					...
					mientras yo-- ) {
						si ((elem = sin igual[i])) {
							semilla[i] = !(coincidencias[i] = elemento);
						}
					}
				}):
				función (elemento, contexto, xml) {
			...
			función de retorno (elemento) {
				...
				hacer {
					if ((elemLang = documentIsHTML ?
						elem.lang :
						elem.getAttribute("xml:idioma") || elem.getAttribute("idioma")) ) {
		...
		"habilitado": función (elemento) {
			devuelve elem.deshabilitado === falso;
		},

		"deshabilitado": función (elemento) {
			return elem.deshabilitado === verdadero;
		},
		...
		"marcado": función (elemento) {
			...
			return (nodeName === "input" && !!elem.checked) || (NodeName === "opción" && !!elemento.seleccionado);
		},
		...
		"padre": función (elemento) {
			return !Expr.pseudos["vacío"]( elemento );
		},
		...
		"último": createPositionalPseudo(función(matchIndexes, longitud) {
			retorno [longitud - 1];
		}),

		"eq": createPositionalPseudo(function(matchIndexes, longitud, argumento) {
...
tokenize = Sizzle.tokenize = función (selector, parseOnly) {
	...
	mientras ( hasta ahora ) {
		...
		for (escriba Expr.filter) {
			if ( (coincidencia = matchExpr[ tipo ].exec( hasta ahora )) && (!preFilters[ tipo ] ||
...
function addCombinator(comparador, combinador, base) {
	var dir = combinador.dir,
		checkNonElements = base && dir === "nodopadre",
		hechoNombre = hecho++;
	...
	devolver combinador.primero?
		...
		función (elemento, contexto, xml) {
			...
			si ( xml ) {
				...
				while ((elemento = elemento[dirección])) {
					if ( elem.nodeType === 1 || checkNonElements ) {
						cachéexterior = elem[ expandir ] || (elemento[ expandir ] = {});
...
function setMatcher( prefiltro, selector, comparador, postfiltro, postFinder, postSelector ) {
	...
	return markFunction(función( semilla, resultados, contexto, xml ) {
		var temp, yo, elemento,
			...
			elementos = semilla || multipleContexts( selector || "*", context.nodeType ? [ context ] : context, [] ),
...
función matcherFromTokens (tokens) {
	var checkContext, comparador, j,
		...
		emparejadores = [función (elemento, contexto, xml) {
			return ( !leadingRelative && ( xml || context !== externalmostContext ) ) || (
...
función matcherFromGroupMatchers(elementMatchers, setMatchers) {
	var bySet = setMatchers.longitud > 0,
		...
		superMatcher = función (semilla, contexto, xml, resultados, exterior) {
			...
			// Mantenga `i` como una cadena si no hay elementos para que `matchedCount` sea "00" debajo
			// Soporte: IE<9, Safari
			// Tolerar propiedades de NodeList (IE: "longitud"; Safari: <número>) elementos coincidentes por ID
			for ( ; i !== len && (elem = elems[i]) != null; i++ ) {
				si (porElemento && elem) {
					j = 0;
					while ((coincidencia = elementoMatchers[j++]) ) {
						si (comparador (elemento, contexto, xml)) {
							resultados.push( elem );
							descanso;
						}
				...
				si (porConjunto) {
					...
					if ( (elemento = !comparador && elemento) ) {
						cuentaCoincidencia--;
					}
			...
			if ( bySet && i !== cuentacoincidencia ) {
				...
				si ( semilla ) {
					...
					if (contador de coincidencias > 0) {
						mientras yo-- ) {
							if ( !(no coincidente[i] || conjuntoMatched[i]) ) {
...
compile = Sizzle.compile = function( selector, match /* Solo para uso interno */ ) {
	...
	si (! en caché) {
		...
		i = partido.longitud;
		mientras yo-- ) {
			almacenado en caché = matcherFromTokens (coincidencia [i]);
		...
		almacenado en caché = compilerCache (selector, matcherFromGroupMatchers (elementMatchers, setMatchers));
...
seleccionar = Sizzle.select = función (selector, contexto, resultados, semilla) {
	var i, tokens, token, tipo, encontrar,
		...
		coincidencia = !seed && tokenize( (selector = compilado.selector || selector) );
	...
	if (coincidencia.longitud === 1) {
		...
		// Tome un atajo y configure el contexto si el selector raíz es una ID
